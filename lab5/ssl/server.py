            ...
            for client in clients:
                if client != client_socket:
                    try:
                        ...
                        clients.remove(client) # Xóa client nếu gửi lỗi
    except: # catch all exceptions during communication
        clients.remove(client_socket) # Xóa client nếu có lỗi trong quá trình xử lý
    finally:
        print("Đã ngắt kết nối:", client_socket.getpeername())
        clients.remove(client_socket) # Đảm bảo client bị xóa khi kết thúc
        client_socket.close()
